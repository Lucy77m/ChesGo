...
import chess
from resource import piece_images
...
class ChessBoard(QWidget):
    ...
    def update_board(self):
        for row in range(8):
            for col in range(8):
                piece = self.main_window.chess_engine.get_board().piece_at(chess.square(col, 7 - row))
                button = self.buttons[(row, col)]
                if piece:
                    pixmap = QPixmap(piece_images[piece.symbol()])
    ...
    def handle_move(self, from_row, from_col, to_row, to_col):
        ...
        if self.main_window.chess_engine.move(move):
            self.update_board()
            self.main_window.chess_engine.make_ai_move()
            self.update_board()
